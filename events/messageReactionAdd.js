...
module.exports = async(client,reaction, user) => {
        ...
        if(data[`${id}`]) {
            if (reaction._emoji.name == "❌") {
                if(data[id].owner === user.id) {
                    await reaction.message.channel.messages.fetch(reaction.message.id).then(async(message) => {
                        await message.reactions.removeAll().then(async() => {
                            await message.edit({
                                embeds:[
                                    new EmbedBuilder() //бля хм
                                        .setTitle(message.embeds[0].data.title)
                                        ...
                                        .setFields(
                                            ...
                                            {
                                                name: `Результат`, value: `${message.embeds[0].data.fields[2].value}`, inline: false
                                            }
                                        )
                                        .setFooter({text:"Опитування закінчено"})
                                ]
                            })
                            delete data[id]
                            fs.writeFile("./voting.json", JSON.stringify(data), function(err) {
                                if(err) return console.log(err)
                            })
                        })
                    })
                    ...
                    await reaction.message.channel.messages.fetch(reaction.message.id).then(async(message) => {
                        await message.reactions.resolve("❌").users.remove(user)
                    })
                }
            }else if (reaction._emoji.name == "🇦"||reaction._emoji.name == "🇧"||reaction._emoji.name == "🇨"||reaction._emoji.name == "🇩"||reaction._emoji.name == "🇪") { 
                await reaction.message.channel.messages.fetch(reaction.message.id).then(async(message) => {
                    ...
                    } //блять а нашо вони треба якщо у нас спеціальний метод для цього
